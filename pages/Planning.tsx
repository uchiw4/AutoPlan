...
export const PlanningPage: React.FC = () => {
    ...
    const handleConfirmLesson = async () => {
        ...
        if (student && instructor) {
            notificationSent = await NotificationService.sendConfirmation(selectedLesson, student, instructor, settings);
        }

        // Ne marquer comme confirmé que si l'envoi de la notification a réussi
        if (notificationSent) {
            ...
            // Si l'envoi a échoué, on garde le modal ouvert et on affiche un message
            setLoading(false);
            // Le message d'erreur est déjà affiché par NotificationService.sendConfirmation
            // Le bouton "Notifier" reste disponible pour réessayer
        }
    };
